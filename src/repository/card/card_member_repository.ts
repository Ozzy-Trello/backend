...
export class CardMemberRepository implements CardMemberRepositoryI {
  ...
  async addMembersToCard(card_id: string, user_ids: string[]): Promise<Array<{id: string, name: string, email: string}>> {
    ...
    for (const user_id of user_ids) {
      await db.insertInto('card_member')
  ...
  async removeMemberFromCard(card_id: string, user_id: string): Promise<void> {
    await db.deleteFrom('card_member')
  ...
  async isCardExist(card_id: string): Promise<boolean> {
    const card = await db.selectFrom('card').where('id', '=', card_id).select('id').executeTakeFirst();
    return !!card;
  }

  async isUserExist(user_id: string): Promise<boolean> {
    const user = await db.selectFrom('user').where('id', '=', user_id).select('id').executeTakeFirst();
    return !!user;
  }

  async isMember(card_id: string, user_id: string): Promise<boolean> {
    const member = await db.selectFrom('card_member')
      .where('card_id', '=', card_id)
      .where('user_id', '=', user_id)
      .select('id')
      .executeTakeFirst();
  ...
  async getExistingMemberIds(card_id: string, user_ids: string[]): Promise<string[]> {
    const members = await db
      .selectFrom('card_member')
      .where('card_id', '=', card_id)
