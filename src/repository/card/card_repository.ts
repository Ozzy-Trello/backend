      ...
      switch (filter.type) {
        case 'board':
          if (filter.operator !== 'any' && filter.value) {
            ...
            } else if (filter.operator === 'includes_any_of' && Array.isArray(filter.value)) {
          ...
          if (filter.operator !== 'any' && filter.value) {
            if (filter.operator === 'includes_any_of') {
              if (Array.isArray(filter.value)) {
                query = query.where('card_member.user_id', 'in', filter.value);
              } else {
          ...
          if (filter.id && filter.operator !== 'any' && filter.value !== undefined) {
            ...
            switch (filter.operator) {
              ...
              case 'contains':
                ...
                break;
              case 'starts_with':
                query = query.where('card_custom_field.custom_field_id', '=', filter.id)
                           .where('card_custom_field.value_string', 'like', `${filter.value}%`);
                break;
              case 'ends_with':
                query = query.where('card_custom_field.custom_field_id', '=', filter.id)
                           .where('card_custom_field.value_string', 'like', `%${filter.value}`);
