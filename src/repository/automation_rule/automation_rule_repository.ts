      ...
      for (const key in filter.condition) {
        ...
        if (val !== undefined) {
          ...
          if (
            ...
            where[Op.or].push(
              literal(
                `condition->>'${EnumSelectionType.NumberComparison}' = '${EnumOptionsNumberComparisonOperators.Exactly}'`
              )
            );
            where[Op.or].push(
              literal(
                `condition->>'${EnumSelectionType.NumberComparison}' = '${EnumOptionsNumberComparisonOperators.FewerThan}'`
              )
            );
            where[Op.or].push(
              literal(
                `condition->>'${EnumSelectionType.NumberComparison}' = '${EnumOptionsNumberComparisonOperators.MoreThan}'`
              )
            );
            where[Op.or].push(
              literal(
                `condition->>'${EnumSelectionType.Action}' = '${EnumUserActionEvent.CardAddedTo}'`
              )
            );
          }
        }
