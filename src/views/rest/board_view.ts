import {
  ...
  BoardFilter,
  UpdateBoardData,
} from "@/controller/boards/board_interfaces";
...
export default class BoardRestView implements BoardRestViewI {
  ...
  async CreateBoard(req: Request, res: Response): Promise<void> {
    let accResponse = await this.board_controller.CreateBoard(
      req.auth!.user_id,
      new BoardCreateData({
  ...
  async GetBoard(req: Request, res: Response): Promise<void> {
    let accResponse = await this.board_controller.GetBoard(
      new BoardFilter({
        id: req.params.id?.toString(),
        workspace_id: req.header("workspace-id")?.toString(),
        workspace_user_id_owner: req.header("my-default")
          ? req.auth?.user_id!
          : undefined,
      })
    );
  ...
  async GetListBoard(req: Request, res: Response): Promise<void> {
    ...
    const userId = req.query.user_id?.toString() || req.auth?.user_id;

    let accResponse = await this.board_controller.GetListBoard(
      new BoardFilter({
        workspace_id: req.header("workspace-id")?.toString(),
        workspace_user_id_owner: req.header("my-default") ? userId : undefined,
      }),
      paginate,
      userId
    );
  ...
  async UpdateBoard(req: Request, res: Response): Promise<void> {
    let updateResponse = await this.board_controller.UpdateBoard(
      new BoardFilter({
        workspace_id: req.header("workspace-id")?.toString(),
        id: req.params.id?.toString(),
        workspace_user_id_owner: req.header("my-default")
  ...
  async DeleteBoard(req: Request, res: Response): Promise<void> {
    let delResponse = await this.board_controller.DeleteBoard(
      new BoardFilter({
        id: req.params.id?.toString(),
        workspace_id: req.header("workspace-id")?.toString(),
        workspace_user_id_owner: req.header("my-default")
