...
import { AutomationRuleControllerI, AutomationRuleCreateData, AutomationRuleFilter } from "@/controller/automation_rule/automation_rule_interface";
...
export default class AutomationRuleRestView implements AutomationRuleRestViewI {
	...
	async CreateAutomationRule(req: Request, res: Response): Promise<void> {
		try {
			const login_response: ResponseData<any> = await this.controller.CreateAutomationRule(req.auth!.user_id, new AutomationRuleCreateData({
				type: req.body.type,
	...
	async GetListAutomationRule(req: Request, res: Response): Promise<void> {
      ...
      let paginate = new Paginate(page, limit);
			...
			const list_response: ResponseData<any> = await this.controller.GetListAutomationRule(new AutomationRuleFilter({
