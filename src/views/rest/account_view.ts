...
export default class AccountRestView implements AccountRestViewI {
  ...
  async GetMyAccount(req: Request, res: Response): Promise<void> {
    let accResponse = await this.account_controller.GetAccount(
      new AccountFilter({ user_id: req.auth!.user_id })
    );
  ...
  async UpdateAccount(req: Request, res: Response): Promise<void> {
    let updateResponse = await this.account_controller.UpdateAccount(
      new AccountFilter({
  ...
  async UpdateMyAccount(req: Request, res: Response): Promise<void> {
    let updateMyAccResponse = await this.account_controller.UpdateAccount(
      new AccountFilter({
    ...
    if (updateMyAccResponse.status_code !== StatusCodes.OK) {
      if (
        updateMyAccResponse.status_code === StatusCodes.INTERNAL_SERVER_ERROR
      ) {
  ...
  async DeleteAccount(req: Request, res: Response): Promise<void> {
    let delResponse = await this.account_controller.DeleteAccount(
      new AccountFilter({
        email: req.query.email?.toString(),
        user_id: req.auth!.user_id,
        username: req.query.username?.toString(),
  ...
  async GetAccount(req: Request, res: Response): Promise<void> {
    const accountResponse = await this.account_controller.GetAccount(
      new AccountFilter({
        user_id: req.params.id,
      })
    );
    if (accountResponse.status_code !== StatusCodes.OK) {
      if (accountResponse.status_code === StatusCodes.INTERNAL_SERVER_ERROR) {
        res.status(accountResponse.status_code).json({
          message: "internal server error",
        });
        return;
      }
