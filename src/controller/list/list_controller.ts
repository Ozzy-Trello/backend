...
export class ListController implements ListControllerI {
  ...
  async CreateList(user_id: string, data: ListCreateData): Promise<ResponseData<CreateListResponse>> {
    ...
    let checkBoard = await this.list_repo.getList({ board_id: data.board_id, name: data.name });
  ...
  async GetListList(filter: ListFilter, paginate: Paginate): Promise<ResponseListData<Array<ListResponse>>> {
    let errorFiled =  filter.getErrorfield();
    if (errorFiled){
    ...
    let boards = await this.list_repo.getListList(filter.toFilterListDetail(), paginate);
    return new ResponseListData({
      message: "List list",
  ...
  async UpdateList(filter: ListFilter, data: UpdateListData): Promise<ResponseData<null>> {
    if (filter.isEmpty()) {
      return new ResponseData({
    ...
    if (filter.id) {
      ...
      let checkBoard = await this.list_repo.getList({ __notId: filter.id, __orName: data.name });
      if (checkBoard.status_code == StatusCodes.OK) {
        return new ResponseData({
    ...
    const updateResponse = await this.list_repo.updateList(filter.toFilterListDetail(), data.toListDetailUpdate());
