...
import {filterUserDetail, UserDetail, UserDetailUpdate} from "@/repository/user/user_interfaces";

export interface AccountControllerI {
	GetAccount(filter: AccountFilter): Promise<ResponseData<AccountResponse>>
	GetAccountList(filter: AccountFilter, paginate: Paginate): Promise<ResponseListData<Array<AccountResponse>>>
...
export function fromUserDetailToAccountResponse(data: UserDetail): AccountResponse {
...
export function fromUserDetailToAccountResponseList(data: Array<UserDetail>): Array<AccountResponse> {
...
export class UpdateAccountData {
	...
	isEmpty(): boolean{
		return this.phone == undefined && this.email == undefined && this.username == undefined
	}
...
export class AccountFilter {
	...
	isEmpty(): boolean{
		return this.user_id == undefined && this.email == undefined && this.username == undefined && this.phone == undefined;
	}
}
